...
export async function createGrant(formData: FormData) {
  ...
  await prisma.grant.create({
    data: {
      ...
      issuingBody: parsed.data.issuingBody, grantType: parsed.data.grantType,
      minAmount: parsed.data.minAmount, maxAmount: parsed.data.maxAmount,
      ...
      approvedByAdmin: isAdmin, status: isAdmin ? "PUBLISHED" : "DRAFT",
    },
  });
  return { success: true };
}
...
export async function getMatchingGrants(companyId: string) {
  ...
  return grants.filter((grant) => isGrantEligible(
    { eligibleAtecoCodes: grant.eligibleAtecoCodes, eligibleRegions: grant.eligibleRegions, eligibleCompanySizes: grant.eligibleCompanySizes, status: grant.status, deadline: grant.deadline?.toISOString() ?? null },
